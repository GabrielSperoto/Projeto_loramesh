...
//         TensaoDeSaida = (((float)valorPot / 4095.0) * 3.3);
        ...
        {
            char buf[32];
            snprintf(buf, sizeof(buf), "Pot: %d  %.2fV", valorPot, TensaoDeSaida);
            displayline(3, buf);
        }
        #endif

        // Envia leitura para a fila de comunicação (para que a CommTask envie pela rede)
        {
            msg_t msg;
            meset(&msg, 0, sizeof(msg_t));
            msg.dst = LOCAL_ADDRESS; // router (1)
            msg.src = loramesh.mydd.devaddr;
            msg.seqnum = lorameshm.mydd.seqnum; // usa o seqnum atual
            msg.function = FCT_READING;
            msg.start = POT;
            msg.qtdParametros = 1;
            msg.size = sizeof(uint32_t); // envia 4 bytes (uint32)
            // Escala a tensão por 100 para manter compatibilidade com o parser no router
            msg.payload.value = (uint32_t)(TensaoDeSaida * 100.0);

            if (xQueueSend(q_app2comm, &msg, 0) != pdTRUE) {
                log_e("Falha ao enviar leitura para q_app2comm");
            } else {
                log_d("Leitura enviada para q_app2comm: %u (scaled)", msg.payload.value);
            }
        }
