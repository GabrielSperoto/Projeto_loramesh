...
strDevicedescription devid[]={
   ...
   {0x4C95,DEV_TYPE_ENDDEV,2,2},
   {0x9C97,DEV_TYPE_ENDDEV,4,4},
    ...
    switch (message->function) {
        ...
        case FCT_READING:
            if (mydd.devtype == DEV_TYPE_ROUTER) { // Requisição
              ...
              buffer[pos++] = message->size;
              // Envia payload em ordem MSB -> LSB para corresponder ao getPayloadValue
              for (int i = message->size; i > 0; i--) {
                buffer[pos++] = message->payload.bytes[i - 1];
              }
            }
            break;
    }
